...
public:
    ...
    int play()
    {
        auto start = chrono::steady_clock::now(); // переменная для хранения времени начала игры

        // проверка на повтор игры
        if (is_replay)
        {
            // повторная инициализация логики, загрузка конфигурации и перерисовка экрана
            logic = Logic(&board, &config);
            config.reload(); // загрузка настроек из settings.json
            board.redraw();
        }
        else
        {
        ...
        bool is_quit = false; // выход из игры?
        const int Max_turns = config("Game", "MaxNumTurns"); // макс. число ходов из настроек (по умолчанию 120)

        // основной цикл игры
        while (++turn_num < Max_turns)
        {
            beat_series = 0;
            logic.find_turns(turn_num % 2); // определяем чей ход (белых/черных)
            ...
            // ход игрока
            if (!config("Bot", string("Is") + ((turn_num % 2) ? "Black" : "White") + "Bot"))
            {
                ...
                {
                    if (config("Bot", string("Is") + ((1 - turn_num % 2) ? "Black" : "White") + "Bot") &&
        ...
        auto end = chrono::steady_clock::now(); // окончание партии
...
private:
    void bot_turn(const bool color) // ход бота
    {
        ...
        thread th(SDL_Delay, delay_ms); // задержка хода
