...
private:
    ...
    double calc_score(const vector<vector<POS_T>> &mtx, const bool first_bot_color) const // функци€ подсчета отчков: передаем массив €чеек и цвет
    {
        ...
        if (!first_bot_color) // если цвет бота не соответствует - мен€ем значени€
        {
            swap(b, w);
            swap(bq, wq);
        }
...
private:
    void find_turns(const bool color, const vector<vector<POS_T>> &mtx) // метод поиска хода
    {
        ...
        for (POS_T i = 0; i < 8; ++i) 
        {
            for (POS_T j = 0; j < 8; ++j)
            {
                if (mtx[i][j] && mtx[i][j] % 2 != color) // если цвет €чейки не подходит
                {
                    find_turns(i, j, mtx);
  ...
  public:
    ...
    int Max_depth; // максимальна€ глубина биты
  ...
  private:
    ...
    Config *config;// указатель на конфигурационна€ файл (setting.json) 
};
