...
public:
    ...
    void redraw() // перерисовка экрана при вызове "повторить игру"
    {
        game_results = -1;
        history_mtx.clear();
    ...
    void move_piece(const POS_T i, const POS_T j, const POS_T i2, const POS_T j2, const int beat_series = 0) // движение шашки
    {
        if (mtx[i2][j2])
    ...
    void drop_piece(const POS_T i, const POS_T j) // убрать шашку
    {
        mtx[i][j] = 0;
        rerender();
    }

    void turn_into_queen(const POS_T i, const POS_T j) // превращение в дамку
    {
    ...
    void highlight_cells(vector<pair<POS_T, POS_T>> cells) // подсветка доступных клеток
    {
        for (auto pos : cells)
        {
    ...
    void clear_highlight() // убрать подсветку
    {
        for (POS_T i = 0; i < 8; ++i)
    ...
    void set_active(const POS_T x, const POS_T y) // выбрать активную шашку
    {
        active_x = x;
        active_y = y;
        rerender();
    ...
    void clear_active() // убрать активную шашку
    {
        active_x = -1;
        active_y = -1;
        rerender();
    ...
    void reset_window_size() // обновить размер окна
    {
        SDL_GetRendererOutputSize(ren, &W, &H);
        rerender();
    }

    void quit() // удалить текстуры при выходе
    {
...
private:
    ...
    void make_start_mtx() // стартовая расстановка
    {
        for (POS_T i = 0; i < 8; ++i)
        {
            for (POS_T j = 0; j < 8; ++j)
    ...
    void rerender() // полная перерисовка
    {
        SDL_RenderClear(ren);
        ...
        for (POS_T i = 0; i < 8; ++i)
        {
            for (POS_T j = 0; j < 8; ++j)
            {
                ...
                switch (mtx[i][j]) {
                    case 1: piece_texture = w_piece; break;
                    case 2: piece_texture = b_piece; break;
                    case 3: piece_texture = w_queen; break;
                    case 4: piece_texture = b_queen; break;
                }
...
private:
    ...
    SDL_Texture *w_queen = nullptr, *b_queen = nullptr;
    SDL_Texture *back = nullptr, *replay = nullptr;
