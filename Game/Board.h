...
public:
    ...
    // draws start board
    int start_draw() // начало рисования экрана
    {
        ...
        if (W == 0 || H == 0) // откуда начинать рисовать экран
        {
            SDL_DisplayMode dm;
            if (SDL_GetDesktopDisplayMode(0, &dm)) // если не можем открисовать экран выдаем ошибку 
            {
        ...
        win = SDL_CreateWindow("Checkers", 0, H / 30, W, H, SDL_WINDOW_RESIZABLE); // создаем окно игры
        if (win == nullptr)
        {
            print_exception("SDL_CreateWindow can't create window");
            return 1;
        }
        ren = SDL_CreateRenderer(win, -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC); // рендеринг окна
        if (ren == nullptr) // если не получилось отрисовать выдаем ошибку
        {
            print_exception("SDL_CreateRenderer can't create renderer");
            return 1;
        }

        // загружаем изображения элементов игры
        board = IMG_LoadTexture(ren, board_path.c_str()); // экран стола
        ...
        w_queen = IMG_LoadTexture(ren, queen_white_path.c_str()); // дама белых
        b_queen = IMG_LoadTexture(ren, queen_black_path.c_str()); // дама черных
        back = IMG_LoadTexture(ren, back_path.c_str()); // кнопка назад
        replay = IMG_LoadTexture(ren, replay_path.c_str()); // кнопка повторить игру
        if (!board || !w_piece || !b_piece || !w_queen || !b_queen || !back || !replay) // проверяем загрузку всех элементов
        {
            print_exception("IMG_LoadTexture can't load main textures from " + textures_path); // ошибка если не загрузили 
            return 1;
        }
    ...
    void redraw() // функция перерисовки экрана при вызове повторить игру 
    {
        game_results = -1;
        history_mtx.clear();
    ...
    void move_piece(move_pos turn, const int beat_series = 0) // движение шашки
    {
        if (turn.xb != -1)
        {
    ...
    void move_piece(const POS_T i, const POS_T j, const POS_T i2, const POS_T j2, const int beat_series = 0) // движение шашки из позиции 1 в позицию 2
    {
        if (mtx[i2][j2])
        {
    ...
    void drop_piece(const POS_T i, const POS_T j) // убирать шашку
    {
        mtx[i][j] = 0;
        rerender();
    }

    void turn_into_queen(const POS_T i, const POS_T j) // изменение шашки на дамку
    {
    ...
    void highlight_cells(vector<pair<POS_T, POS_T>> cells) // выбор шашки 
    {
        for (auto pos : cells)
        {
            POS_T x = pos.first, y = pos.second;
            is_highlighted_[x][y] = 1; // выделение шашки
        }
        rerender();
    }

    void clear_highlight() // снятие выделении шашки
    {
        for (POS_T i = 0; i < 8; ++i)
        {
    ...
    void set_active(const POS_T x, const POS_T y) // выбор активной шашки
    {
        active_x = x;
        active_y = y;
        rerender();
    ...
    void clear_active()  // убрать активность шашки
    {
        active_x = -1;
        active_y = -1;
        rerender();
    ...
    bool is_highlighted(const POS_T x, const POS_T y) // шашка выделена
    {
        return is_highlighted_[x][y];
    }

    void rollback() // вернуть шашку обратно
    {
    ...
    void reset_window_size() // изменение экрана
    {
        SDL_GetRendererOutputSize(ren, &W, &H);
        rerender();
    }

    void quit() // уничтожение текстур при выходе из игры
    {
...
private:
    void add_history(const int beat_series = 0) // добавление истории
    {
